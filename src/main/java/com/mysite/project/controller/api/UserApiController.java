...
public class UserApiController {
	...
	//회원수정
	@PutMapping("/user")
	public ResponseDto<Integer> update(@RequestBody Users users, HttpSession session){
//		System.out.println("api controller user : " );
//		System.out.print( "user : : @@@ :::" + user);
		userService.userUpate(users);
		
		//여기서는 트랜잭션이 종료되기 때문에 DB에 값은 변경이 됐음.
		//하지만 세션값은변경되지 않은 상태이기 때문에 직접 세션값을 변경해줄거임

		//세션등록
		UserDetails userDetail = principalDetailService.loadUserByUsername(users.getUsername());
